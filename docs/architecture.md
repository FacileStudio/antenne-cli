...
Nook's API authenticates by cookie and reads nothing else — no bearer header, no API key. So
`nook login` posts the password, reads `Set-Cookie: nook_session=…` off the response, and
...
An instance running with no `NOOK_ADMIN_PASSWORD` serves every caller as the admin and sets
...
Nook serves its dashboard from the same origin as its API, and the SPA catch-all answers an
...
*"answered with something that is not JSON — check the URL points at a Nook instance"*. It
is the same class of trap as a health check that only asserts `status < 400`.

## The event stream

`nook tail` reads server-sent events, not a WebSocket: the traffic is one-way, and the
instance already exposes `/api/events/stream` for its own dashboard.

`internal/client/sse.go` implements only the part of the grammar Nook emits — `data:` lines
...
`nook test` and `nook replay` take a target id or a name. The match is exact id, then exact
...
noise in somebody's chat room, and `nook targets` is one command away.

## What this client does not do

It reads configuration and never writes it. There is no `nook providers add`, no
`nook settings set`.

That is a deliberate boundary, not a missing feature. Nook's settings are one document
